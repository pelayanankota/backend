...
function insertPengaduan($data) {
    ...
    $jp = htmlspecialchars($data["nik"]);
    $dp = htmlspecialchars($data["no"]);
    $nb = htmlspecialchars($data["layanan"]);
    $ks = htmlspecialchars($data["sifat"]);
    $ia = htmlspecialchars($data["informasi"]);
    $ga = htmlspecialchars($data["aduan"]);
    $pp = htmlspecialchars($data["petugas"]);
    $ket = mysqli_real_escape_string($conn, $data["ket"]);
    $status = "Sedang diajukan";
    $ket_petugas = "-";
    $tgl_lapor = date("Y-m-d");

    mysqli_query($conn, "INSERT INTO pengaduan VALUES('$id', '$np', '$jp', '$dp', '$nb', '$ks', '$ia', '$ga', '$pp', '$ket', '$status', '$ket_petugas', '$tgl_lapor')");
